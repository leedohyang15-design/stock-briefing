...
# (그룹, 표시명, yfinance 티커, 소수점 자리, 스케일)
# 스케일: 원시 종가에 곱할 배수 (원/엔은 JPYKRW=X × 100 = '100엔당 원').
...
_INDICES = [
    ...
    ("해외", "S&P 500", "^GSPC", 2, 1),
    ("환율", "원/달러", "KRW=X", 2, 1),
    ("환율", "원/엔(100엔)", "JPYKRW=X", 2, 100),
    ("안전자산·원자재", "금", "GC=F", 2, 1),
    ("안전자산·원자재", "WTI 유가", "CL=F", 2, 1),
    ("안전자산·원자재", "미 장기국채(TLT)", "TLT", 2, 1),
    ("안전자산·원자재", "미 단기국채(SHY)", "SHY", 2, 1),
]
...
def fetch_indices() -> List[IndexQuote]:
    ...
    for group, name, ticker, ndigits, scale in _INDICES:
        try:
            ...
            quotes.append(
                IndexQuote(group=group, name=name, value=round(last * scale, ndigits),
